...
const ResetPassword = ({ token }: { token: string }) => {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        const res = await api.post<{ email: string }>("/api/forgot-password/verify-token", {
          token,
        });
        ...
        const msg = (error as { response?: { data?: string } })?.response?.data ?? "Invalid or expired link.";
        toast.error(msg);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await api.post<string>("/api/forgot-password/update", {
        email: user?.email,
      ...
      const msg = (error as { response?: { data?: string } })?.response?.data ?? "Failed to update password.";
      toast.error(msg);
      setLoader(false);
    }
  };
